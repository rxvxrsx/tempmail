...
def ver_check():
    ...
    while True:
        try:
            mainver = requests.get("https://raw.githubusercontent.com/cyfer-xk/TempMail/main/.version?token=GHSAT0AAAAAACPUBFEIDEYSJPQBPJPBL4WSZP2Z7EQ").text
            break
        except:
    ...
    if not (vernow == mainver):
        psb("\n    [*] Tool Update Found...")
        psb("    [*] Updating Tool...")
        os.system("cd .. && rm -f TempMail && git clone https://github.com/cyfer-xk/TempMail > /dev/null 2>&1")
        psb("\n    [*] Tool Update Complete...")
        psb("    [*] Starting Tool...")
        time.sleep(0.7)
        os.system("cd .. && cd TempMail && python mail.py")
    else:
